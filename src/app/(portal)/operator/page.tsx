...
//
// Two ways to a workspace: pick an organisation from the list, or open one by its id.
// The second matters: a closed account has no organisation profile, so it is not in the
// list, yet a data-erasure request for it can still arrive. The list can also fail on
// its own, and that must never leave an operator with nowhere to go.
...
export default function OperatorConsolePage() {
  ...
  return (
    <div>
      ...
      <form onSubmit={openWorkspace} className="mb-8 max-w-xl rounded-2xl border border-black/[0.08] p-5">
        <label htmlFor="open-workspace" className="block text-[13px] font-semibold text-black">Open a workspace</label>
        <p className="mt-0.5 text-[12px] text-gray-500">
          By its id, for example <span className="font-mono">org-84a4d521</span>. Works for closed accounts that no longer appear in the list.
        </p>
      ...
      {listFailed ? (
        <div role="alert" className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-[13px] text-amber-800">
          ...
          You can still open a workspace by its id above.
        </div>
      ) : orgs.length === 0 ? (
